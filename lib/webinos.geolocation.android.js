...
// store watch ids in this table under given key from caller
  ...
  try {
    androidImpl = require('bridge').load(require("../platform_interfaces.json").android.GeolocationImpl, this);
  } catch (e) {
  ...
  function watchPosition(args, successCB, errorCB, objectRef) {
    ...
    watchIdTable[objectRef.rpcId] = androidImpl.watchPosition(function (position) {
      ...
      if (false === rpcHandler.executeRPC(rpc)) {
        console.log("client disconnected - clearing listener " + objectRef.rpcId);
        clearWatch([objectRef.rpcId]);
      }
    }, function (err) {
